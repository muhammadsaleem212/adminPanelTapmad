...
@section('action-content')
  ...
  <div class="box-header">
    <div class="row">
        ...
        <div class="col-sm-4">
          <a class="btn btn-primary" href="{{ route('userPermissions.create') }}">Add new user Screen Permission</a>
        </div>
    </div>
  </div>
  <!-- /.box-header -->
  <div class="box-body">
      ...
      <form method="POST" action="{{ route('userPermissions.search') }}">
         {{ csrf_field() }}
      </form>
          ...
          <table id="example2" class="table table-bordered table-hover dataTable">
            ...
            @foreach ($userPermission as $userPermissions)
                <tr class="odd">
                  ...
                  <td>
                    <form class="row" method="POST" action="{{ route('userPermissions.destroy', ['id' => $userPermissions->id]) }}" onsubmit = "return confirm('Are you sure?')">
                        <input type="hidden" name="_method" value="DELETE">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <a href="{{ route('userPermissions.edit', ['id' => $userPermissions->id]) }}" class="btn btn-warning col-sm-3 col-xs-5 btn-margin">
                        Update
                        </a>
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/datatables/1.10.19/css/dataTables.bootstrap.min.css"></script>
