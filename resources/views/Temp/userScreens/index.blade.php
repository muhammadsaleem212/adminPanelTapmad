...
@section('action-content')
  ...
  <div class="box-header">
    <div class="row">
        ...
        <div class="col-sm-4">
          <a class="btn btn-primary" href="{{ route('userScreens.create') }}">Add new user Screen</a>
        </div>
    </div>
  </div>
  <!-- /.box-header -->
  <div class="box-body">
      ...
      <form method="POST" action="{{ route('userScreens.search') }}">
         {{ csrf_field() }}
      </form>
          ...
          <table id="example2" class="table table-bordered table-hover dataTable">
            ...
            @foreach ($userScreen as $screens)
                <tr class="odd">
                  ...
                  <td>
                    <form class="row" method="POST" action="{{ route('userScreens.destroy', ['id' => $screens->userScreenID]) }}" onsubmit = "return confirm('Are you sure?')">
                        <input type="hidden" name="_method" value="DELETE">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <a href="{{ route('userScreens.edit', ['id' => $screens->id]) }}" class="btn btn-warning col-sm-3 col-xs-5 btn-margin">
                        Update
                        </a>
