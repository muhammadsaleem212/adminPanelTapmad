...
@extends('TransactionSuccessful.base')
@section('action-content')
  ...
  <div class="box-body">
      ...
      <form method="POST" action="{{url('TransactionSuccessful')}}" enctype="multipart/form-data">
        
         {{ csrf_field() }}
        
         @component('layouts.search', ['title' => 'Search'])
         <div class = "col-md-5">
         <label class="col-md-4 control-label">Transaction Messages</label>
                            <div class="col-md-6">
        ...
        <div class="col-md-8 col-md-offset-2">
          ...
          <div class="box">
            ...
            @foreach ($TransactionSuccessful as $Success)
                <tr class="odd">
                  <td class="sorting_1">{{ $Success->UserPaymentPlatform }}</td>
                  <td class="sorting_1">{{ $Success->UserPaymentUserName }}</td>
                  <td class="sorting_1">{{ $Success->UserPaymentPackageType }}</td>
                  <td class="sorting_1">{{ $Success->UserPaymentOperatorID }}</td>
                  <td class="sorting_1">{{ $Success->UserPaymentMobileNumber }}</td>
                  <td class="sorting_1">{{ $Success->UserPaymentStartDate }}</td>
                  <td class="sorting_1">{{ $Success->UserPaymentMessage }}</td>
              </tr>
            @endforeach
            </tbody>
            </form>
