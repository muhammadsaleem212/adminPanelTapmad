...
@section('action-content')
  ...
  <div class="box-header">
    <div class="row">
        ...
        <div class="col-sm-6">
          <a href="{{url('saveDynamicOffersbyForm')}}" class="btn btn-primary">Add new Dynamic Offer by Form</a>
          <a class="btn btn-success" href="{{url('saveDynamicOffersbyCsv')}}">Add new Dynamic Offer by Csv</a>
        </div>
  ...
  <div class="box-body">
      ...
      <form method="POST" action="{{url('dynamicOffers/search')}}">
         {{ csrf_field() }}
      </form>
          ...
          <div class="box">
            ...
            @foreach ($dynamicOffer as $dynamicOffers)
                <tr class="odd">
                  ...
                  <td>
                    <form class="row" method="POST" action="{{ url ('dynamicOffers/destroy', ['id' => $dynamicOffers->id]) }}" onsubmit = "return confirm('Are you sure?')">
                        <input type="hidden" name="_method" value="DELETE">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <a href="{{ url ('dynamicOffers/edit', ['id' => $dynamicOffers->id]) }}" class="btn btn-warning col-sm-4 col-xs-5 btn-margin">
                        Update
                        </a>
