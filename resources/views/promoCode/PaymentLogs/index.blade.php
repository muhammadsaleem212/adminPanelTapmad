...
@section('action-content')
  ...
  <div class="box-body">
      ...
      <form method="POST" action="{{url('PaymentLogs')}}" enctype="multipart/form-data">
        
         {{ csrf_field() }}
        
         @component('layouts.search', ['title' => 'Search'])
         <div class = "col-md-5">
         <label class="col-md-4 control-label">Payment Messages</label>
                            <div class="col-md-6">
                                <select class="form-control" name="PaymentMessage">
                                    @foreach ($SuccessfulTransactionMessage as $message)
                                        <option value="{{$message->PaymentLogMessage}}">{{$message->PaymentLogMessage}}</option>
                                    @endforeach
                                </select>
                            </div>
        </div>
      ...
      <div class="row">
        ...
        <div class="col-md-8 col-md-offset-2">
          ...
          <div class="box">
            ...
            <tbody>
            @foreach ($TransactionSuccessful as $Success)
                <tr class="odd">
                  <td class="sorting_1">{{ $Success->PaymentLogPlatform }}</td>
                  <td class="sorting_1">{{ $Success->PaymentLogUserId }}</td>
                  <td class="sorting_1">{{ $Success->PaymentLogProductId }}</td>
                  <td class="sorting_1">{{ $Success->PaymentLogOperatorId }}</td>
                  <td class="sorting_1">{{ $Success->PaymentLogMobileNo }}</td>
                  <td class="sorting_1">{{ $Success->PaymentLogAddedDate }}</td>
                  <td class="sorting_1">{{ $Success->PaymentLogMessage }}</td>
              </tr>
            @endforeach
            </tbody>
            </form>
