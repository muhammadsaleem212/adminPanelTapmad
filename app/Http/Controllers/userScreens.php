...
class userScreens extends Controller
{
    ...
    public function store(Request $request)
    {
      ...
      //  print_r($userScreens); exit;
        $this->validate($request, [
            'userScreenName'            => 'required|unique:userscreens',
            'screenDisplayName'         => 'required|unique:userscreens'
        ]);
        $checkUserScreen = userScreen::checkUserScreen($userScreens);
        if($checkUserScreen){ 
            return redirect('saveUserScreens');
       }else{
        $userScreen = array();
        $userScreen['userScreenName']       = $userScreens['userScreenName'];
